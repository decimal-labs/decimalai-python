...
def _print_trust_result(data, base_url, name):
    """Shared success print for the verify/attest trust-tier commands."""
    agg = data.get("aggregate_metrics", {}) or {}
    pr = agg.get("pass_rate", {}) or {}
    delta = pr.get("delta_pts")
    click.echo(
        f"  ✓ {'VERIFIED' if data.get('verified') else 'NOT verified'} "
        f"· {data.get('verification_method')} "
        f"· verdict {data.get('overall_verdict')} "
        f"· {('+' if (delta or 0) > 0 else '')}{delta} pts "
        f"· graded by {data.get('grading_method')}"
    )
    calib = agg.get("expectation_calibration")
    if calib:
        click.echo(
            f"    judged expectations: {calib.get('eligible_count')}/{calib.get('judged_count')} "
            f"count toward lift (the rest are display-only)"
        )
    skipped = agg.get("skipped_cases") or []
    if skipped:
        click.echo(f"  ⚠ {len(skipped)} case(s) skipped: "
                   f"{', '.join(str(s.get('case', s)) for s in skipped[:5])}")
    click.echo(f"  View: {_dashboard_url(base_url)}/skills/{name}?tab=benchmark")


@skills.command("verify")
@click.argument("bundle_file", type=click.Path(exists=True, dir_okay=False))
@click.option("--skill", "skill_name", required=True, help="Skill name to verify (sync it first).")
@click.option("--no-rejudge", is_flag=True, default=False,
              help="Skip judged cases instead of re-judging them with the platform judge (strict zero-LLM).")
@common_options
def skills_verify(bundle_file, skill_name, no_rejudge, api_key, base_url, project):
    """Verify-by-replay: upload publisher-run A/B outputs; the platform RE-GRADES into a VERIFIED run.

    BUNDLE_FILE is the replay bundle from the open-source runner:

    \b
        skillevaluation run ./my-skill --emit-replay-bundle bundle.json
        decimalai skills verify bundle.json --skill my-skill

    Grading is the platform's (you can't fudge it), so the result is verified. Deterministic cases
    re-grade at zero LLM; judged cases are re-judged with the platform judge (--no-rejudge skips them).
    """
    import json as _json
    try:
        payload = _json.loads(open(bundle_file, encoding="utf-8").read())
    except (OSError, ValueError) as e:
        click.echo(f"  ✗ could not read {bundle_file}: {e}", err=True)
        raise SystemExit(1)
    if no_rejudge:
        payload["rejudge"] = False

    client = _make_client(api_key, base_url, project)
    try:
        resp = client._http.post(f"/api/v1/skills/{skill_name}/benchmark/replay", json=payload)
        if resp.status_code == 404:
            click.echo(f"  ✗ skill {skill_name!r} not found — run `decimalai skills sync` first.", err=True)
            raise SystemExit(1)
        if resp.status_code in (400, 422):
            click.echo(f"  ✗ rejected: {(resp.json() or {}).get('detail', resp.text)}", err=True)
            raise SystemExit(1)
        resp.raise_for_status()
        _print_trust_result(resp.json(), base_url, skill_name)
    finally:
        client.close()


# Restricted command: the server accepts this upload only from specifically
# enabled accounts, so it is hidden from `--help` rather than advertised to
# everyone who would only ever get a 402 back. It stays wired up so the
# accounts that do have it can run it.
@skills.command("attest", hidden=True)
@click.argument("attestation_file", type=click.Path(exists=True, dir_okay=False))
@click.option("--skill", "skill_name", required=True, help="Skill name to attest (sync it first).")
@common_options
def skills_attest(attestation_file, skill_name, api_key, base_url, project):
    """Upload a self-run benchmark for a skill. Restricted — most accounts get a 402.

    ATTESTATION_FILE is the payload from the open-source runner:

    \b
        skillevaluation run ./my-skill --emit-attestation attest.json
        decimalai skills attest attest.json --skill my-skill

    The caller ran both generation and the judge on its own key, so the server does not re-run
    the model; it recomputes what it can from the payload (lift, never-hurt, calibration) and
    checks the judge model against its own before storing the result.
    """
    import json as _json
    try:
        payload = _json.loads(open(attestation_file, encoding="utf-8").read())
    except (OSError, ValueError) as e:
        click.echo(f"  ✗ could not read {attestation_file}: {e}", err=True)
        raise SystemExit(1)

    client = _make_client(api_key, base_url, project)
    try:
        resp = client._http.post(f"/api/v1/skills/{skill_name}/benchmark/attest", json=payload)
        if resp.status_code == 404:
            click.echo(f"  ✗ skill {skill_name!r} not found — run `decimalai skills sync` first.", err=True)
            raise SystemExit(1)
        if resp.status_code == 402:
            click.echo("  ✗ this account is not enabled for self-run benchmark uploads.", err=True)
            raise SystemExit(1)
        if resp.status_code in (400, 422):
            click.echo(f"  ✗ rejected: {(resp.json() or {}).get('detail', resp.text)}", err=True)
            raise SystemExit(1)
        resp.raise_for_status()
        data = resp.json()
        _print_trust_result(data, base_url, skill_name)
        pin = (data.get("aggregate_metrics") or {}).get("judge_pin") or {}
        if pin and not pin.get("ok"):
            click.echo("  ⚠ judge/runner did not match the platform canonical — recorded but not "
                       "comparable on the shared leaderboard.")
    finally:
        client.close()


@skills.command("spotcheck")
@click.option("--skill", "skill_name", required=True, help="Skill whose latest replay run to spot-check.")
@click.option("--k", type=int, default=5, help="Number of random cases to re-run on the platform (default 5).")
@click.option("--tolerance", type=float, default=0.2, help="Max arm-mismatch fraction tolerated (default 0.2).")
@common_options
def skills_spotcheck(skill_name, k, tolerance, api_key, base_url, project):
    """Spot-check a replay run: the platform re-runs K random cases on its own key and compares.

    Upgrades a verify-by-replay run to the trustless tier (or un-verifies it if the outputs don't
    reproduce). Uses real LLM (K×2 calls). Run ``decimalai skills verify`` first.
    """
    client = _make_client(api_key, base_url, project)
    try:
        resp = client._http.post(
            f"/api/v1/skills/{skill_name}/benchmark/spotcheck",
            params={"k": k, "tolerance": tolerance},
        )
        if resp.status_code == 404:
            click.echo(f"  ✗ no replay run to spot-check for {skill_name!r} — run "
                       f"`decimalai skills verify` first.", err=True)
            raise SystemExit(1)
        if resp.status_code in (400, 422):
            click.echo(f"  ✗ rejected: {(resp.json() or {}).get('detail', resp.text)}", err=True)
            raise SystemExit(1)
        resp.raise_for_status()
        data = resp.json()
        sc = (data.get("aggregate_metrics") or {}).get("spotcheck") or {}
        click.echo(
            f"  {'✓' if data.get('verified') else '✗'} spot-check {sc.get('verdict', '?')} "
            f"· method {data.get('verification_method')} "
            f"· {sc.get('mismatches', '?')}/{sc.get('arms_checked', '?')} arm-mismatches"
        )
        if not data.get("verified"):
            click.echo("  ⚠ outputs did not reproduce on the platform — the run is now UN-verified "
                       "and dropped from the leaderboard.")
        click.echo(f"  View: {_dashboard_url(base_url)}/skills/{skill_name}?tab=benchmark")
    finally:
        client.close()
