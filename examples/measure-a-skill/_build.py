...
cells = [
    ...
    code(
        ...
        "\n"
        "print(\"\\nNote what the survivors ARE. Most are GitHub imports whose no-skill arm\")\n"
        "print(\"hand-rolled a working alternative because it did not know a CLI existed —\")\n"
        "print(\"technically a fail, but a cold reader looks at that arm and says 'seems fine'.\")\n"
        "print(\"A high delta is not the same as a legible one, which is why the demo skill\")\n"
    ...
    code(
        ...
        "\n"
        "# Deliberately NOT printing case['outcome'] here. On this case it reads\n"
        "# `fail_kept` (both arms failed) while the expectation rows below show the\n"
        "# no-skill arm passing both — the scorecard's headline rule sets aside the\n"
        "# expectations a skill made worse, which folds a regression into the same\n"
        "# label as a case neither arm ever got right. Printing that label beside the\n"
        "# rows it disagrees with would just look like a bug in our own page.\n"
