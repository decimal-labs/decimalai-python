#!/usr/bin/env python3
"""Build the Tier 2 notebook: route_and_trace.ipynb.

TIER 2 OF THREE. Tier 0 audits our evidence with no credentials. Tier 1 re-runs
the benchmark on the reader's own model key. This one is where a DecimalAI
account has to earn its place — and the only way to earn it is to first make the
reader watch their own approach fail on their own key.

THE SHAPE OF THE ARGUMENT. Cell 2.0 runs BEFORE any DecimalAI credential, on the
model key the reader already pasted in Tier 1. It splices three real registry
skills into one system prompt and re-runs the same lane. The reader watches the
prompt bill multiply on every single request and watches an irrelevant policy get
a vote on an employment-law question. Only then does 2.1 ask for a key, and the
ask is a sentence about the numbers they just produced rather than a feature list.

WHAT IT MUST NEVER DO — same contract as Tiers 0 and 1:
  * No credential of any kind means skip notices and exit 0. Never a traceback.
    A notebook that dies on a missing key has taught the reader that we do not
    test the path they are actually on.
  * No hardcoded numbers. Everything is manifest-pinned (examples/measure-a-skill/
    manifest.yaml) or measured live in front of the reader.
  * Print the result whichever way it falls. Cell 2.0 has three verdict branches
    including "the extra skills HELPED", because a demo that can only produce one
    outcome is not a measurement.

THE THING THIS NOTEBOOK REFUSES TO FAKE. On a fresh workspace `/skills/route`
returns `strategy: full_menu` — three skills cost ~120 of its 1,500-token
description budget, so the server returns everything and skips the embedding step.
Ranking does not bind until the menu stops fitting. The tempting move is to splice
"the top result" anyway; under full_menu that list is in NAME order, and our demo
skill happens to sort first, so the table would look like routing worked when it
was alphabetical luck. Cell 2.3 says so out loud and does the selection the honest
way — the load_skill loop the SDK actually ships — which works at three skills and
at three hundred.
"""
import json
import os
...
cells = [
    ...
    md(
        ...
        "examples/measure-a-skill/route_and_trace.ipynb)\n"
        "\n"
    ...
    code(
        ...
        "        # No Gemini 2.x here: retired — generateContent 404s even though\n"
        "        # models.list still returns the ids, so a listing check passes a\n"
        "        # model that every real call will fail on.\n"
    ...
    code(
        ...
        "    # version=None takes whatever is latest — right only for a reader-chosen\n"
        "    # slug, where there is no measured-version pin to honour.\n"
        "    ref = f\"{slug}@{version}\" if version else slug\n"
        "    try:\n"
        "        return get(f\"https://app.decimal.ai/s/{ref}/SKILL.md\")\n"
        "    except Exception as exc:\n"
        "        print(f\"  could not fetch {ref} ({type(exc).__name__}) — \"\n"
        ...
        "\n"
        "# The registry's \"open in Colab\" CTA appends `?skill=<slug>` to this notebook's\n"
        "# URL. Colab cannot hand that to you: cells run on the kernel VM, and the frame\n"
        "# that could run page JavaScript is a different origin from the notebook page, so\n"
        "# the URL parameter is unreadable from here. Paste the slug instead — empty means\n"
        "# the manifest's pinned demo skill.\n"
        "SKILL_SLUG = \"\"\n"
        "\n"
        "TARGET = SKILL_SLUG.strip() or DEMO[\"slug\"]\n"
        "# The demo skill is pinned to the version that was measured; a reader-chosen\n"
        "# slug has no such pin and takes latest.\n"
        "TARGET_VERSION = DEMO[\"version\"] if TARGET == DEMO[\"slug\"] else None\n"
        "if TARGET not in BODIES:\n"
        "    # Either you picked a slug above, or the manifest's gates fell through to the\n"
        "    # warm spare — both leave the pinned splice set without the skill being\n"
        "    # measured. Put it back, first, or the two arms would differ by more than one\n"
        "    # variable.\n"
        "    body = fetch_body(TARGET, TARGET_VERSION)\n"
        "    if body:\n"
        "        BODIES[TARGET] = body\n"
    ...
    code(
        ...
        "        return None\n"
        "    ok, ptok, calls, broke, picks, failed = 0, 0, 0, [], [], []\n"
        "    for c in lane:\n"
        ...
        "        if not passed:\n"
        "            broke.append((c[\"case_name\"],\n"
        "                          f\"model call failed: {finish}\" if text is None\n"
        "                          else why or f\"finish={finish}\"))\n"
        "        if note:\n"
        "            picks.append(note)\n"
        "    if len(failed) == len(lane):\n"
        "        # Every model call in this arm failed, so there are no answers — and no\n"
        "        # answers is NOT a score. Recording it as 0/8 would let a dead model id\n"
        "        # or a spent quota masquerade as \"the skill scored zero\", and every\n"
        "        # cost/accuracy conclusion drawn from the table would be about an\n"
        "        # outage, not about prompts.\n"
        "        print(f\"  {label:<20} no data — all {len(lane)} model calls failed \"\n"
        "              f\"({failed[0]}); arm skipped, not scored\")\n"
        "        return None\n"
    ...
    md(
        ...
        "  there is nothing to instrument — and on the release you just installed, the\n"
        "  combination is not safe: through 0.10.0 (the current PyPI release) the framework\n"
        "  flags are applied *outside* the `enabled` gate, so a disabled init still runs the\n"
        "  LangChain install, which submits a background skill pull that calls the API and\n"
        "  writes `SKILL.md` files to disk. The gate fix ships in 0.10.1: from that version a\n"
        "  disabled init installs no adapter, calls nothing and writes nothing. Until you are\n"
        "  on 0.10.1 or later, don't combine `enabled=False` with any framework flag."
    ),
    code(
...
path = os.path.join(os.path.dirname(__file__), "route_and_trace.ipynb")
