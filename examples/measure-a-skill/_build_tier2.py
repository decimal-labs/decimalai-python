...
cells = [
    ...
    md(
        ...
        "- Your workspace starts **genuinely empty**, on purpose. Signup used to seed a demo\n"
        "  agent — about a hundred traces, a dataset and a regression check written before\n"
        "  your first page render — and it *satisfied* every onboarding surface instead of\n"
        "  teaching it: the getting-started checklist read \"7/7, you're all set\" ninety\n"
        "  seconds after signup, to someone who had installed nothing. The seed still exists\n"
        "  and you can now choose it. Cell 2.2 is what puts your first real thing in the\n"
        "  workspace.\n"
    ...
    md(
        ...
        "  `.stream(...)` is **not**. `langchain.py:252` leaves an unrecognised shape untouched\n"
        "  rather than guessing, which is the right call and also means an LCEL chain silently\n"
        "  gets no skills. If you build with `prompt | llm`, inject the routed fragment into your\n"
        "  own prompt template instead of relying on the patch.\n"
        "\n"
