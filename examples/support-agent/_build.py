...
if SKILL_MD.exists():
    ...
    print("\nThat is the whole product on this axis: about four pages of Markdown you can\n"
          ...
          "and you could have used curl. The eval.yaml beside it is the 25 cases the +68\n"
          "was measured on.")
else:
...
print(textwrap.fill(
    ...
    md(
        ...
        "\n"
        "- `decimalai.install(enable_skill_loader=True)` will do the injection for you and add\n"
        "  tracing — but it authenticates and serves *your workspace's* installed skills, so it\n"
        "  needs an account, which this notebook deliberately does not. Its injection is also\n"
        ...
        "  `invoke` a `PromptValue`, which it leaves untouched (`langchain.py:252`). So the most\n"
...
wrong_arm = ask(TICKET, WRONG)'''.replace(
    ...
    md(
        ...
        "25 cases it had already been graded on.\n"
        "\n"
        ...
        "### Not free\n"
        "\n"
        "The `pull` told you: **+50% tokens, +0% turns**. Four pages of Markdown ride along on\n"
        "every request the skill applies to. Past two or three skills, route them instead of\n"
        "concatenating them.\n"
        "\n"
        "### Next\n"
        "\n"
